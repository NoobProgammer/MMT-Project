...
def handleMakeOrder(make_order, second_frame, data, on_receive_order, check_expiration, extend_order):
    ...
    #More Order
    def handleMoreOrder(check_expiration):
    ...
    if (len(data_order) == 0):
        ...
        tk.Button(second_frame, text="More Order", command=lambda: handleMoreOrder(check_expiration)).place(x=200, y=50, width=100, height=50)
        renderOrder(data)
